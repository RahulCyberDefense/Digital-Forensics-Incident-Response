...
![1_dRE1MNANPhWdRrHOfjn3cQ.png](1_dRE1MNANPhWdRrHOfjn3cQ.png)
...
- I launched Velociraptor (`Velociraptor.exe gui`) and accepted the browser warning.
- I searched the Artifact Exchange for PrintNightmare and found `Windows.Detection.PrintNightmare`.
- I composed a VQL query that reconstructs full paths from the MFT and parses PEs, then filtered for `Windows/System32/spool/drivers` and non-directory PE files.
- The malicious DLL discovered was **`nightmare.dll`**, and its PDB path was **`C:\Users\caleb\source\repos\nightmare\x64\Release\nightmare.pdb`**.
