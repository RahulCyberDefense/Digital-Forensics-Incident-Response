...
*Case 001 - Banking trojan masquerading as Adobe reader*

- I inspected host info and process tree. The suspicious process `reader_sl.exe` (PID 1640) run under `explorer.exe` (PID 1484) stood out. I dumped its memory and recovered the adversary User-Agent: `Mozilla/5.0 (Windows; U; MSIE 7.0; Windows NT 6.0; en-US)`. I also confirmed references to Chase Bank in the process strings.

*Case 002 — Ransomware incident*

- The decryptor `@WanaDecryptor@` (PID 740) was present and executed from `C:\Intel\ivecuqmanpnirkt615\@WanaDecryptor@.exe`, launched by `tasksche.exe` (parent PID 1940). The malware is **WannaCry**, uses `ws2_32.dll` for sockets, and creates the `MsWinZonesCacheCounterMutexA` mutex. For file enumeration in its working directory, use `windows.filescan`.
